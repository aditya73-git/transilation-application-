...
class LanguageService:
    """Manage language pairs and switching"""

    def __init__(self):
        """Initialize language service"""
        config = get_config()

        # Define language pairs (user can modify in config)
        self.supported_languages = config.get_languages()
        self.language_codes = config.get_language_codes()

        # Create language pairs: English with each other language
        self.language_pairs = self._create_language_pairs()
        self.current_pair_index = 0
        self.current_pair = self.language_pairs[0]

        logger.info(f"Language service initialized with {len(self.language_pairs)} pairs")

    def _create_language_pairs(self) -> List[Tuple[str, str]]:
        """Create language pairs (EN source, each other language as target)"""
        pairs = []

        # Bidirectional pairs
        for lang in self.supported_languages:
            if lang != "english":
                pairs.append(("english", lang))
                pairs.append((lang, "english"))
    ...
    def switch_language_next(self):
        ...
        logger.info(f"Language pair switched to: {self.current_pair[0]} → {self.current_pair[1]}")
        return self.current_pair
    ...
    def set_language_pair(self, source: str, target: str) -> bool:
        ...
        if pair in self.language_pairs:
            ...
            logger.info(f"Language pair set to: {source} → {target}")
            return True
        else:
            logger.warning(f"Language pair not found: {source} → {target}")
            return False
