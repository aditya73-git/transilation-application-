...
class FinalBridgeApp {
 public:
  void begin() {
    ...
    Serial.println("ReSpeaker final bridge starting... [button+dial-v1]");
    ...
    Wire.begin(
        half_duplex_bridge_config::kI2CSdaPin,
        half_duplex_bridge_config::kI2CSclPin);
    if (step16_.begin()) {
      dialAvailable_ = true;
      dialLastValue_ = step16_.getValue();
      Serial.printf(
          "STEP16 ready addr=0x%02X initial=%u\n",
          half_duplex_bridge_config::kStep16Address,
          static_cast<unsigned>(dialLastValue_));
    } else {
      Serial.printf(
          "STEP16 not found at 0x%02X\n",
          half_duplex_bridge_config::kStep16Address);
    }
    ...
    Serial.printf(
        "Step16 on I2C addr=0x%02X via SDA=%d SCL=%d\n",
        half_duplex_bridge_config::kStep16Address,
        half_duplex_bridge_config::kI2CSdaPin,
        half_duplex_bridge_config::kI2CSclPin);
 ...
 private:
  ...
  UnitStep16 step16_{half_duplex_bridge_config::kStep16Address, &Wire};
  ...
  unsigned long dialLastPollMs_ = 0;
  ...
  bool dialAvailable_ = false;
  uint8_t dialLastValue_ = 0;
  ...
  static void micCaptureTaskEntry(void* arg) {
    static_cast<FinalBridgeApp*>(arg)->micCaptureTask();
  }
  ...
  void serviceDial() {
    ...
    if ((now - dialLastPollMs_) < half_duplex_bridge_config::kDialPollMs) {
      return;
    }
    dialLastPollMs_ = now;

    const uint8_t value = step16_.getValue();
    if (value == dialLastValue_) {
      return;
    }

    dialLastValue_ = value;
    Serial.printf("STEP16 -> %u (0x%X)\n", static_cast<unsigned>(value), static_cast<unsigned>(value));
    char message[16];
    snprintf(message, sizeof(message), "DIAL %u\n", static_cast<unsigned>(value));
    sendButtonEvent(message);
  }
